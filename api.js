...
router.get("/user/:username", (req, res) => {
    ...
    return index.mysql.promise().query("SELECT id, login, balance, uuid FROM accounts WHERE login = ?", [req.params.username])
        .then((user) => {
            if(!user[0][0])
                return res.status(404).json({ error: { code: 404, message: "Игрок не найден." } });
            else {
                let userGroup = index.mysql.promise().query("SELECT username, primary_group FROM luckperms_players WHERE username = ?", [req.params.username.toLowerCase()])
                .then((rank) => {

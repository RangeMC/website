    ...
    account.promise().query("SELECT * FROM accounts WHERE lk_cookie = ?", [req.cookies.loginHash])
        .then(async (user) => {
            if(!user[0][0]) {
                ...
                let userGroup = await luckperms.promise().query("SELECT * FROM luckperms_players WHERE username = ?", [user[0][0].login.toLowerCase()]);
    ...
    account.promise().query("SELECT * FROM accounts WHERE lk_cookie = ?", [req.cookies.loginHash])
        .then(async (user) => {
            if(!user[0][0]) {
                ...
                else return account.promise().query("SELECT * FROM accounts")
        ...
        .then((user) => {
            if(!user[0][0]) {
                ...
                if(req.query.accept == "true")
                    return account.promise().query(`SELECT * FROM ${(req.query.type == "skin") ? "skins" : (req.query.type == "cloak") ? "cloaks" : "undefined"} WHERE login = ?`, [req.params.nickname])
                        .then((item) => {
                            if(!item[0][0]) return res.redirect("/panel/list");
                            else return account.promise().query(`UPDATE ${(req.query.type == "skin") ? "skins" : (req.query.type == "cloak") ? "cloaks" : "undefined"} SET hash = ? WHERE login = ?`, [item[0][0].hash, user[0][0].login])
                                .then(() => {
                                ...
                                }).catch(() => account.promise().query(`INSERT INTO ${(req.query.type == "skin") ? "skins" : (req.query.type == "cloak") ? "cloaks" : "undefined"} (hash, login) VALUES (?, ?)`, [item[0][0].hash, user[0][0].login])
                                    .then(() => {
                                        // rcon.send(`kick ${user[0][0].login} @${(req.query.type == "skin") ? "Skin" : (req.query.type == "cloak") ? "Cloak" : "undefined"}Changed`);
                                        res.redirect(`/panel/event?type=Success${(req.query.type == "skin") ? "Skin" : (req.query.type == "cloak") ? "Cloak" : "undefined"}Change`);
                                    }).catch(console.error)
                ...
                else return account.promise().query("SELECT * FROM accounts WHERE login = ?", [req.params.nickname])
                    .then((accounts) => {
    ...
    account.promise().query("SELECT * FROM accounts WHERE lk_cookie = ?", [req.cookies.loginHash])
        .then(async (user) => {
            if(!user[0][0]) {
                ...
                let userGroup = await luckperms.promise().query("SELECT * FROM luckperms_players WHERE username = ?", [user[0][0].login.toLowerCase()]);
                if(!["admin"].includes(userGroup[0][0].primary_group)) return res.redirect("/panel/event?type=AccessDenied");
                else return account.promise().query("SELECT * FROM accounts WHERE login = ?", [req.params.login])
                    .then((accounts) => {
...
router.post("/addUser", (req, res) => {
    account.promise().query("SELECT * FROM accounts WHERE lk_cookie = ?", [req.cookies.loginHash])
        .then(async (user) => {
            ...
            let userGroup = await luckperms.promise().query("SELECT * FROM luckperms_players WHERE username = ?", [user[0][0].login.toLowerCase()]);
            if(!["admin"].includes(userGroup[0][0].primary_group)) return res.redirect("/panel/event?type=AccessDenied");
            else return account.promise().query("SELECT * FROM accounts WHERE login = ?", [req.body.login])
                .then((accounts) => {
                    if(accounts[0][0]) return res.redirect("/panel/admin");
                    else return account.promise().query("INSERT INTO accounts (login, pass) VALUES (?, ?)", [req.body.login, req.body.pass])
            ...
            if(!["admin"].includes(userGroup[0][0].primary_group)) return res.redirect("/panel/event?type=AccessDenied");
                ...
                .then((accounts) => {
                    ...
                    if(accounts[0][0].access < 4)
                        return account.promise().query("UPDATE accounts SET login = ?, pass = ?, access = ?, blocked = ?, invited_by = ?, vk = ? WHERE login = ?", [req.body.login, req.body.pass, req.body.access, req.body.blocked, ((req.body.invited_by.length <= 3) ? null : req.body.invited_by), req.body.vk, req.body.login])
                            .then(() => {
...
router.get("/deleteUser", (req, res) => {
    account.promise().query("SELECT * FROM accounts WHERE lk_cookie = ?", [req.cookies.loginHash])
        .then(async (user) => {
            ...
            let userGroup = await luckperms.promise().query("SELECT * FROM luckperms_players WHERE username = ?", [user[0][0].login.toLowerCase()]);
            if(!["admin"].includes(userGroup[0][0].primary_group)) return res.redirect("/panel/event?type=AccessDenied");
            else return account.promise().query("SELECT * FROM accounts WHERE login = ?", [req.query.login])
                .then((accounts) => {
                    if(!accounts[0][0]) return res.redirect("/panel/admin");
                    else {
                        if(accounts[0][0].access < 4)
                            return account.promise().query("DELETE FROM accounts WHERE login = ?", [req.query.login])
        ...
        .then((user) => {
            if(!user[0][0]) return res.redirect("/");
                ...
                uploadSkin(req, res, function (err) {
                    ...
                    } else return account.promise().query("SELECT * FROM skins WHERE login = ?", [user[0][0].login])
                        .then((skin) => {
                            if(!skin[0][0]) return account.promise().query("INSERT INTO skins (hash, login) VALUES (?, ?)", [req.file.filename, user[0][0].login])
                                .then(() => {
                                    // rcon.send(`kick ${user[0][0].login} @SkinChanged`);
                                    return res.redirect("/panel/event?type=SuccessSkinChange");
                                }).catch(console.error);
                            else return account.promise().query("UPDATE skins SET hash = ? WHERE login = ?", [req.file.filename, user[0][0].login])
                                .then(() => {
                                    // rcon.send(`kick ${user[0][0].login} @SkinChanged`);
        ...
        .then((user) => {
            if(!user[0][0]) return res.redirect("/");
                ...
                uploadCloak(req, res, function (err) {
                    ...
                    } else return account.promise().query("SELECT * FROM cloaks WHERE login = ?", [user[0][0].login])
                        .then((skin) => {
                            if(!skin[0][0]) return account.promise().query("INSERT INTO cloaks (hash, login) VALUES (?, ?)", [req.file.filename, user[0][0].login])
                                .then(() => {
                                    // rcon.send(`kick ${user[0][0].login} @CloakChanged`);
                                    res.redirect("/panel/event?type=SuccessCloakChange");
                                }).catch(console.error);
                            else return account.promise().query("UPDATE cloaks SET hash = ? WHERE login = ?", [req.file.filename, user[0][0].login])
                                .then(() => {
                                    // rcon.send(`kick ${user[0][0].login} @CloakChanged`);
                                    res.redirect("/panel/event?type=SuccessCloakChange");
                                }).catch(console.error);
        ...
        .then((user) => {
            if(!user[0][0]) return res.redirect("/");
                ...
                if(user[0][0].pass !== req.body.passold) return res.redirect("/panel/password?error=IncorrectPassword");
                    ...
                    account.promise().query("UPDATE accounts SET pass = ? WHERE login = ?", [req.body.passnew, user[0][0].login]);
...
router.post("/login", (req, res) => {
    account.promise().query("SELECT * FROM accounts WHERE login = ?", [req.body.login])
        .then((user) => {
            if(!user[0][0]) return res.redirect("/panel/login?error=NotFound");
                ...
                if(user[0][0].pass !== req.body.pass) return res.redirect("/panel/login?error=IncorrectPassword");
                    ...
                    if(user[0][0].blocked == 1) { // return res.redirect("/panel/login?error=Deactivated");
                        ...
                        account.promise().query("UPDATE accounts SET lk_cookie = ? WHERE login = ?", [hash, user[0][0].login]);
    ...
    account.promise().query("SELECT * FROM accounts WHERE login = ?", [req.body.login])
        .then((user) => {
            ...
            if(!req.recaptcha.error) {
                ...
                account.promise().query("INSERT INTO accounts (login, pass, lk_cookie) VALUES (?, ?, ?)", [req.body.login, req.body.pass, hash]);
                return res.redirect("/panel");
...
router.get("/logout", (req, res) => {
    ...
    account.promise().query("SELECT * FROM accounts WHERE lk_cookie = ?", [req.cookies.loginHash])
        .then((user) => {
            ...
            account.promise().query("UPDATE accounts SET lk_cookie = ? WHERE lk_cookie = ?", [null, req.cookies.loginHash])
                .catch(console.error);
